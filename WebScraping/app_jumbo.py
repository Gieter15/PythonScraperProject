...
class JumboProductsScraper():
    ...
    def __init__(self, headless=False) -> None:
        ...
        while not last_page:
            ...
            print("***page number {0}/{1} ***".format(page_number, number_of_pages))
            ...
            for i, html_product in enumerate(page_products):
                ...
                if product:
                    ...
                    print('({}/{})\t€{},{}\t{}.'.format(i+1, len(page_products), product.price_int, product.price_frac,product.title))
                elif product == None:
    ...
    def analyze_html_product(self, html_product, page_number) -> Product:
        ...
        if p.product_id:
            p.id = p.product_id + str(datetime.now().isocalendar()[0]) + str(datetime.now().isocalendar()[1]) + str(datetime.now().isocalendar()[2])
        return p

    def store_products_in_database(self, products: list[Product], db_folder, db_name, db_table_name) -> None:
