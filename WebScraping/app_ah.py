...
class AhProductsScraper():
    ...
    def __init__(self, headless) -> None:
        ...
        for url in urls:
            ...
            try:
                popup_message = driver.find_element_by_xpath(
                    "//button[@class='popover_closeButton__2FHcJ']")
                popup_message.click()
            except:
            ...
            self.store_products_as_csv(
                all_products, csv_folder, csv_file_prefix)

            print('{0} products analyzed, inserts: {1}, updates: {2}, untouched: {3}'.format(
                i+1, self.inserts, self.updates, self.untouched))
    ...
    def analyze_html_product(self, html_product):
        ...
        for line in lines:
            if not line.upper().isupper():
                try:
                    ...
                    print(
                        '***problem with obtaining price of product from string {0}. skipping***'.format(lines))
        ...
        if re.search(sale_1, product_text):  # EXAMPLE:  "[0-9] + [0-9] GRATIS"
            ...
            p.set_price(unit_price)
            p.sale = 1
        # "[0-9][0-9]\% KORTING , correct price is already obtained in analyzing lines"
        elif re.search(sale_3, product_text):
            p.sale = 1
        ...
        try:
            p.product_id = int([u for u in p.url.split(
                '/') if u.startswith('wi')][0][2::])
        except:
        ...
        for p in products:
            try:
                ...
                if p.price_int == -1 or p.price_frac == -1:
                    continue
                elif p.product_id != -1 and p.product_id not in product_ids:
    ...
    def find_product_category_links(self, input_driver):
        ...
        while True:
            try:
                category_container = input_driver.find_element_by_class_name(
                    "product-category-overview_root__2Lyn0")
                categories = category_container.find_elements_by_tag_name(
                    "div")
    ...
    def find_cookies_button(self, input_driver):
        ...
        while True:
            try:
                cookies_button = input_driver.find_element_by_id(
                    'accept-cookies')
            except:
                if retry_nr < self.max_tries:
    ...
    def find_products(self, input_driver):
        ...
        while True:
            try:
                container = input_driver.find_element_by_class_name(
                    "search-lane-wrapper")
    ...
    def find_url(self, input_product):
        ...
        while True:
            try:
                url = input_product.find_element_by_tag_name(
    ...
    def find_product_title(self, input_product):
        ...
        while True:
            try:
                prd_title = input_product.find_element_by_tag_name(
